...
<div class="row justify-content-center flex-fill gx-3 my-5">
  {#each topPlayers as player, index}
    <div class="col-4 order-{getOrder(index)}">
      <div class="h-100 d-flex flex-column align-items-center justify-content-end pb-7">
        <div class="font-cursive lead">{player.name}</div>
        <div class="display-2 font-cursive">{player.total}</div>
        <div
          class="bg-base-4 w-100 {getHeight(index)} d-flex justify-content-center align-items-end"
        >
